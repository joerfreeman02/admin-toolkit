...
7. **Publish the Employee Viewer.** Keep the configured Employee Viewer access code unless you deliberately need to rotate it. Confirm the month is ready, select **Publish Employee Viewer**, wait for **Employee Viewer published**, then select **Copy employee-view link**. Send the normal Outlook hyperlink and the access code separately. No download, GitHub, deployment or developer involvement is part of this monthly step.
...
Source workbooks are read locally and are never rewritten by NEXUS. Never place timesheets, Employee Register data, generated reports, access codes or confidential workbooks in GitHub. The optional encrypted backup is for recovery and is not part of normal publishing. Each publication has its own random ID, so publishing September does not alter August. Rotating the access code does not decrypt older publications: retain an old code or republish the required month from source data.
