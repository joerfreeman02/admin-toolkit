...
The parser also accepts a deliberately simple synthetic `Timesheet` table with `Employee`, `Month`, `Project Code`, `Description`, optional `Category`, and `Hours` for deterministic fixtures. Formula handling uses cached cell values available in the browser library and also sums daily cells where a cached monthly total is absent. Cached/calculated formula differences remain an explicit warning/validation concern.
...
Aggregation groups by code (or uncoded description), then employee, without discarding row lineage in the admin result. Reconciliation asserts total = project + internal + exception. Missing/blank employee records remain warnings/audit state and create no hours.
